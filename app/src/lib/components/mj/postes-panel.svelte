...
<Card.Root class="xl:col-span-2">
	...
	<Card.Content class="space-y-1.5">
		{#each posts as post (post.clientId)}
			<div class="flex flex-wrap items-center gap-2 rounded-lg border border-border px-2.5 py-1.5">
				<span class="w-6 shrink-0 text-center font-mono text-sm font-bold">{post.number}</span>
				<span
					class="size-2.5 shrink-0 rounded-full {post.connected ? 'bg-green-500' : 'bg-muted'}"
				...
				{/if}
				<select
					class="h-7 rounded-md border border-input bg-input/30 px-1.5 text-xs"
					value={post.role ?? ''}
				...
				<Button
					...
					title="Retire ce poste du registre (numéro réattribuable). Une machine encore vivante se ré-enregistrera à son prochain rechargement."
		...
		{/each}
		{#if posts.length === 0}
			<p class="text-sm text-muted-foreground">Aucun poste enregistré pour l'instant.</p>
		{/if}
	</Card.Content>
</Card.Root>
