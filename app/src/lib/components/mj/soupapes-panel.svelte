		...
		<div>
			...
			<div class="max-h-96 space-y-2 overflow-y-auto pr-1">
				{#each posts as post (post.clientId)}
					...
					<div id={`hint-${post.clientId}`} class="rounded-lg border border-border p-2">
						...
						{#if hint}
							<p class="mb-1.5 text-sm text-foreground/80">{hint.text}</p>
						{/if}
						{#if draft}
							<div class="flex flex-wrap items-end gap-2">
								<textarea
									id={`hint-text-${post.clientId}`}
									class="h-14 min-w-48 flex-1 rounded-md border border-input bg-input/30 px-2 py-1 text-sm"
									placeholder="Texte de l'indice…"
									bind:value={draft.text}></textarea>
								<div class="flex flex-col gap-1">
									<Label for={`hint-level-${post.clientId}`} class="text-xs text-muted-foreground"
										>Niveau</Label
									>
									<select
